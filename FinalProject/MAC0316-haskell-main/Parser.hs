...
import Tokenizer (isNumeral, isSymbol, isReservedWord, isForbiddenSymbol, numFirst)
import Types (Token, ParseTree (..), ExprS (..))

-- | O parser transforma uma lista de tokens em uma árvore sintática.
...
analyze tree = case tree of
  ...
  Pair first rest -> case first of
    ...
    Leaf "case"   -> CaseS (analyzePos 1) (analyzePos 3) (analyzePos 4)
    ...
    where
      ...
      getSymbol i = case tree `index` i of
        Leaf symbol
          | isReservedWord symbol -> error "ERRO analyze: identificador é uma palavra reservada"
          | isForbiddenSymbol symbol -> error "ERRO analyze: identificador começa com símbolo inválido"
          | numFirst symbol -> error "ERRO analyze: identificador começa com número"
          | otherwise -> symbol
