...
tokenize str@(char : chars)
  ...
  | isNumeralChar char = accumulate (\c -> isSymbolChar c || isNumeralChar c)
  | isSymbolChar char = accumulate (\c -> isSymbolChar c || isNumeralChar c)
...
-- | Lista de todos os caracteres válidos para símbolos da linguagem.
--
-- Lembrando que uma `String` é uma lista de `Char`.
symbols :: String
symbols = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_!?-+*/%<>#~"

-- | Lista de todos os dígitos válidos para números da linguagem.
numerals :: String
numerals = "0123456789."
...
-- e criando uma nova função que só recebe o caractere a ser buscado na coleção.
...
-- Nossas funções
forbiddenSymbols = "()_!?-+*%<>#~"
reservedWorlds = ["lambda", "call", "if", "let", "letrec", "cons", "head", "tail", "case", "of"]

isReservedWord :: String -> Bool
isReservedWord word = word `elem` reservedWorlds

isForbiddenSymbol :: String -> Bool
isForbiddenSymbol (char:string) = char `elem` forbiddenSymbols

numFirst :: String -> Bool
numFirst (char:string) = char `elem` numerals
