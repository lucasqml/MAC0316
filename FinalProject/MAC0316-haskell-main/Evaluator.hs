...
eval exp env = case exp of
  ...
  QuoteC sym -> SymV sym
  CaseC val b1 b2 -> caseRec val (eval b1 env) (eval b2 env)
    where
      caseRec val intList codeList =
        case (eval val env, intList, codeList) of
          (NumV num, ConsV (NumV int1) (NumV int2), ConsV code1 code2) ->
            if num == int1
              then code1
            else if num == int2
              then code2
              else error "não era para ter chegado aqui"
          (NumV num, ConsV (NumV int) intList, ConsV code codeList) ->
            if num == int
              then code
              else caseRec val intList codeList
