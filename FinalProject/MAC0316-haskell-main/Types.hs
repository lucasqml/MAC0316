...
-- de desaçucarização.
--
-- Esta árvore representa uma expressão do código da linguagem interpretada.
...
data ExprS
  ...
  | -- | Representa um desvio condicional n-direcional
    CaseS    { valueS :: ExprS, intS :: ExprS, codeS :: ExprS }
  deriving (Show, Eq)
...
data ExprC
  ...
  | -- | Representa um desvio condicional n-direcional
    CaseC { valueC :: ExprC, intC :: ExprC, codeC :: ExprC }
  deriving (Show, Eq)
